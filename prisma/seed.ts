import { PrismaClient, ReadinessStatus, ScreenStatus } from "@prisma/client";
...
async function main() {
  ...
  for (const campus of campusDefinitions) {
    ...
    locations.set(campus.code, location.id);
    ...
    await prisma.siteReadiness.upsert({
      ...
      create: {
        ...
        power: ReadinessStatus.NOT_STARTED,
        network: ReadinessStatus.NOT_STARTED,
        mounting: ReadinessStatus.NOT_STARTED,
        player: ReadinessStatus.NOT_STARTED,
      },
    });
    ...
    for (let i = 1; i <= campus.screens; i += 1) {
      ...
      await prisma.screen.upsert({
        ...
        create: { name: `${campus.name} Display ${i}`, externalId, locationId: location.id, status: ScreenStatus.UNKNOWN },
      });
    }
  }
  ...
  const email = process.env.SEED_ADMIN_EMAIL ?? "admin@taletso.edu.za";
